...
function EditGoalWindow({ goal, handleClose, handleCreateNewGoal }) {
    const [errors, setErrors] = useState({})
    const titleRef = useRef()
    const prizeRef = useRef()

    const [checkedItems, setCheckedItems] = useState(goal.targets.map(target => target._id))
    ...
    const { data: progresses, updateGoal } = useProgressesStore()
            ...
            <motion.div
                ...
                <div className='px-5 grid grid-cols-2 gap-y-4'>
                    <div className='col-span-2 flex flex-col gap-y-1'>
                        <input ref={titleRef} defaultValue={goal.title} type="text" className={`text-input ${errors?.title && 'outline outline-2 outline-red-600'}`} placeholder='Title' />
                        {errors?.title && (<span className='text-xs text-red-600'>{errors.title}</span>)}
                    </div>
                    <div className='col-span-2 flex flex-col gap-y-1'>
                        <input ref={prizeRef} type="text" defaultValue={goal.prize} className={`text-input ${errors?.prize && 'outline outline-2 outline-red-600'}`} placeholder='Prize' />
                ...
                <button
                    ...
                    className='m-5 py-2 rounded-xl bg-blue-600 text-white text-lg'>
                    Update
                </button>
